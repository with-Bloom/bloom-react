    ...
    <ul className="flex flex-col gap-3">
      {options.map((el) => (
        <div key={el} className="relative">
          <button
            className={`flex h-11 w-[327px] items-center justify-items-start rounded-[5px] bg-gray100 p-[13px] py-6 font-Pretendard font-medium text-gray700 hover:bg-indigo ${
              el === '3분' ? 'relative' : ''
            }`}
            onClick={() => onClick(el)}
          >
